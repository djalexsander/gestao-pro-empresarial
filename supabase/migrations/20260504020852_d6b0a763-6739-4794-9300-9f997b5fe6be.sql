-- Atualiza finalizar_venda_pdv para aceitar data_vencimento (necessário para vendas FIADO)
-- e validar que vendas FIADO tenham cliente + vencimento.
DROP FUNCTION IF EXISTS public.finalizar_venda_pdv(
  uuid, numeric, numeric, numeric, forma_pagamento, text, numeric, numeric, text,
  jsonb, jsonb, boolean, uuid, uuid, uuid
);
...
BEGIN
  ...
  FOR v_pgto IN SELECT * FROM jsonb_array_elements(v_pagamentos) LOOP
    IF (v_pgto->>'forma_pagamento') = 'fiado' THEN
      v_tem_fiado := true;
    END IF;
  ...
  -- ===== Validação FIADO: exige cliente + vencimento =====
  IF v_tem_fiado THEN
