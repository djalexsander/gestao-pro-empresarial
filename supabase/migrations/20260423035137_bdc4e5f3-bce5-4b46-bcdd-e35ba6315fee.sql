...
-- 1) Adicionar coluna caixa_id em financeiro_lancamentos
ALTER TABLE public.financeiro_lancamentos
  ADD COLUMN IF NOT EXISTS caixa_id UUID REFERENCES public.caixas(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_lanc_caixa ON public.financeiro_lancamentos(caixa_id);

-- 2) Atualizar função fechar_caixa para gerar lançamentos no financeiro
...
BEGIN
  ...
  -- Gerar lançamentos no Financeiro (entradas já recebidas) por forma de pagamento
