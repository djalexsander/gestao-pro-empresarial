...
const dashboard: DataAdapter["dashboard"] = {
  ...
  async kpiDetalhe({ tipo, inicio, fim }) {
    ...
    if (tipo === "vendas") {
      const { data: vendas } = await supabase
        ...
        .neq("status", "cancelada")
        .order("data_emissao", { ascending: false });
      const ids = [...new Set((vendas ?? []).map((v) => v.cliente_id).filter(Boolean) as string[])];
      ...
      return {
        rows,
        resumo: [
          { label: "Total vendido", valor: formatBRL(total), tone: "success" as const },
          { label: "Quantidade de vendas", valor: String(rows.length), tone: "info" as const },
          {
            label: "Ticket médio",
    ...
    if (tipo === "compras") {
      const { data: compras } = await supabase
        ...
        .order("data_emissao", { ascending: false });
      const ids = [
        ...new Set((compras ?? []).map((c) => c.fornecedor_id).filter(Boolean) as string[]),
      ];
      ...
      const map = new Map((forn ?? []).map((f) => [f.id, f.nome_fantasia || f.razao_social]));
      ...
      return {
        rows,
        resumo: [
          { label: "Total comprado", valor: formatBRL(total), tone: "info" as const },
          { label: "Quantidade de compras", valor: String(rows.length), tone: "info" as const },
        ],
      };
    }
    ...
    if (tipo === "lucro") {
      ...
      const totalVendas = (vendasRes.data ?? []).reduce((s, v) => s + Number(v.total ?? 0), 0);
      ...
      const rows = Array.from(porMes.entries())
        .sort(([a], [b]) => (a < b ? 1 : -1))
        .map(([k, val]) => {
      ...
      return {
        rows,
        resumo: [
          { label: "Receita total", valor: formatBRL(totalVendas), tone: "success" as const },
          { label: "Custo total", valor: formatBRL(totalCompras), tone: "info" as const },
          {
            label: "Lucro do período",
            valor: formatBRL(lucro),
            tone: (lucro >= 0 ? "success" : "danger") as "success" | "danger",
          },
          {
            label: "Margem",
        ...
        resumo: [
          {
            ...
            tone: (isPagar ? "warning" : "success") as "warning" | "success",
          },
          { label: "Quantidade", valor: String(rows.length), tone: "info" as const },
          {
            label: "Vencidos",
            valor: `${vencidos.length} (${formatBRL(vencidos.reduce((s, r) => s + r.valor, 0))})`,
            tone: (vencidos.length > 0 ? "danger" : "muted") as "danger" | "muted",
          },
        ],
      };
    }
      ...
      resumo: [
        {
          ...
          tone: (rows.length > 0 ? "danger" : "muted") as "danger" | "muted",
        },
        {
          label: "Zerados",
...
import { cloudRelatoriosAdapter } from "./cloud-relatorios";
...
export const cloudAdapter: DataAdapter = {
  ...
  relatorios: cloudRelatoriosAdapter,
};
